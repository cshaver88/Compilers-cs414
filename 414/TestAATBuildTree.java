...
class TestAATBuildTree {
	...
	public static void main(String args[]) {
		...
		System.out
				.println("------------------------------------------------------------");
		stmtree = bt.forStatement(
				bt.assignmentStatement(bt.baseVariable(8),
						bt.constantExpression(0)),
				bt.operatorExpression(bt.baseVariable(8),
						bt.constantExpression(10), AATOperator.LESS_THAN),
				bt.assignmentStatement(
						bt.baseVariable(8),
						bt.operatorExpression(bt.baseVariable(8),
								bt.constantExpression(1), AATOperator.PLUS)),
		...
		System.out.println("------------------------------------------");
		System.out
				.println("FuncDef: int foo() { int y; int x = 1; } (&x = -4)");
		System.out.println("------------------------------------------");
		functiondefs = bt.functionDefinition(
				bt.assignmentStatement(bt.baseVariable(-4),
						bt.constantExpression(1)), 8, new Label(), new Label());
		functiondefs.Accept(pt);
